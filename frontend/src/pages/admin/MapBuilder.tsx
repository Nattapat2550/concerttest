...
export default function MapBuilder({ mapConcert, onBack }: MapBuilderProps) {
 ...
 className={`p-3 border rounded cursor-pointer transition-all ${!isEraserMode && activeChannelId === ch.id ? 'border-primary ring-2 ring-blue-200 shadow-md bg-blue-50' : 'border-outline'}`}
 >
 ...
 <p className="text-sm text-muted mt-6 bg-lifted p-3 rounded">
 <span className="flex items-center font-bold text-ink mb-2">
 <img src={ideaImg} alt="Idea" className="w-5 h-5 mr-2 object-contain" /> 
 ทิปส์การใช้งาน:
 </span>
 • <b>ลากคลุม:</b> กด <kbd className="bg-gray-300 px-1 rounded text-ink">Shift</kbd> ค้างไว้แล้วลากเมาส์<br/>
 • <b>เลือกทั้งโซน:</b> คลิกที่กรอบเส้นโซนบนแผนที่<br/>
 • <b>ซูมเข้า/ออก:</b> กด <kbd className="bg-gray-300 px-1 rounded text-ink">Ctrl</kbd> ค้าง + เลื่อนลูกกลิ้ง
 </p>
 </div>
