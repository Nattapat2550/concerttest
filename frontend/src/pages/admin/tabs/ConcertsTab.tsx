...
const inputStyle = "w-full p-3 bg-canvas border border-outline text-ink text-sm rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-primary outline-none transition-all";
const labelStyle = "block text-sm font-bold text-ink mb-1.5";
...
export default function ConcertsTab({ onOpenMapBuilder }: ConcertsTabProps) {
 ...
 {/* ---------------- ฟอร์มสร้างคอนเสิร์ต ---------------- */}
 <form onSubmit={handleCreateConcert} className="bg-canvas p-6 sm:p-8 rounded-lg border border-outline shadow-sm relative overflow-hidden">
 ...
 <input type="text" disabled placeholder="สร้างอัตโนมัติ" className={`${inputStyle} bg-lifted text-muted cursor-not-allowed`} />
 </div>
 ...
 isHtmlModeNew ? 'bg-primary text-white shadow-purple-500/30' : 'bg-canvas text-ink border border-outline'
 }`}
 >
 ...
 <div className="bg-canvas rounded-lg overflow-hidden border border-outline ">
 {isHtmlModeNew ? (
 <textarea 
 ...
 *คุณสามารถระบุ URL ของรูปภาพตั๋วในช่อง `bgUrl` และปรับแกน X, Y เพื่อกำหนดตำแหน่งการวาง QR Code หรือหมายเลขที่นั่งได้
 ...
 {concerts.map(c => (
 <div key={c.id} className="bg-canvas rounded-lg border border-outline shadow-sm hover:shadow-md transition-shadow overflow-hidden flex flex-col h-full relative">
 
 <div className="flex justify-between items-center p-4 border-b border-outline bg-canvas/50 /50">
 <span className="font-mono text-xs font-bold text-muted bg-lifted px-2.5 py-1 rounded-sm">ID: {c.id}</span>
 {c.is_active ? (
 ...
 <p className="text-sm text-muted flex items-center gap-2"><img src={placeImg} className="w-4 h-4 opacity-60" alt="Venue" /><span className="truncate">{c.venue_name || c.venue || 'ไม่ระบุ'}</span></p>
 <p className="text-sm text-muted flex items-center gap-2"><img src={calendarImg} className="w-4 h-4 opacity-60" alt="Date" />{new Date(c.show_date).toLocaleString('th-TH', { dateStyle: 'medium', timeStyle: 'short' })} น.</p>
 <p className="text-sm text-muted flex items-center gap-2"><img src={ticketImg} className="w-4 h-4 opacity-60" alt="Price" />เริ่มต้น ฿{c.ticket_price?.toLocaleString() || 0}</p>
 </div>
 </div>
 
 <div className="p-4 bg-canvas/80 border-t border-outline grid grid-cols-3 gap-2">
 ...
 <div className="sticky top-0 bg-canvas z-10 px-6 py-4 border-b border-outline flex justify-between items-center">
 ...
 <button type="button" onClick={() => setEditingConcert(null)} className="text-muted hover:text-ink text-2xl font-bold p-2 leading-none">&times;</button>
 </div>
 ...
 <div className="flex items-center bg-lifted rounded-lg overflow-hidden border border-outline">
 <span className="px-4 py-3 bg-lifted text-muted font-bold border-r border-outline">ID</span>
 <input type="text" disabled value={editingConcert.access_code} className="w-full p-3 bg-transparent text-ink font-mono font-bold outline-none" />
 </div>
 </div>
 ...
 className={`flex items-center gap-2 px-4 py-2 rounded-lg text-sm font-bold transition-all shadow-sm ${
 isHtmlModeEdit ? 'bg-primary text-white' : 'bg-canvas text-ink border border-outline'
 }`}
 >
 ...
 <div className="p-6 border-t border-outline bg-canvas flex justify-end gap-3 rounded-b-2xl">
 <button type="button" onClick={() => setEditingConcert(null)} className="px-6 py-3 bg-canvas border border-outline text-ink rounded-lg font-bold hover:bg-lifted">ยกเลิก</button>
