...
export default function HomePage() {
  ...
  return (
    <div className="w-full overflow-x-hidden bg-canvas  pb-20 transition-colors duration-300">
      ...
      <div className="relative bg-canvas  text-ink  overflow-hidden border-b border-gray-200 ">
        ...
        <div className="w-full px-6 lg:px-12 2xl:px-20 py-24 lg:py-32 relative z-10 flex flex-col md:flex-row items-center justify-between gap-12">
          <div className="text-center md:text-left flex-1">
            ...
            <h1 className="text-5xl md:text-7xl lg:text-8xl font-semibold mb-6 leading-tight tracking-tighter">
              ...
              <span className="text-gray-400 ">Live Music</span> Journey
            </h1>
            ...
            <Link to="/concerts" className="inline-flex items-center gap-2 bg-black  text-white  font-semibold py-4 px-10 rounded-full hover:scale-105 transition-transform duration-300">
      ...
      {isCarouselsLoading ? (
        ...
        <div className="w-full px-6 lg:px-12 2xl:px-20 mt-10">
          <div className="relative w-full h-62.5 md:h-100 lg:h-112.5 rounded-[2rem] overflow-hidden shadow-2xl bg-canvas  border border-gray-100 ">
            ...
            {carousels.length > 1 && (
              <>
                <button onClick={() => setCurrentSlide(prev => (prev - 1 + carousels.length) % carousels.length)} className="absolute left-4 top-1/2 -translate-y-1/2 z-20 bg-canvas/50 hover:bg-canvas /50 dark:hover:bg-black backdrop-blur-md text-black  w-12 h-12 rounded-full transition-all flex items-center justify-center font-bold shadow-lg">
                  <ChevronLeft size={24} />
                </button>
                <button onClick={() => setCurrentSlide(prev => (prev + 1) % carousels.length)} className="absolute right-4 top-1/2 -translate-y-1/2 z-20 bg-canvas/50 hover:bg-canvas /50 dark:hover:bg-black backdrop-blur-md text-black  w-12 h-12 rounded-full transition-all flex items-center justify-center font-bold shadow-lg">
      ...
      <div className="w-full px-6 lg:px-12 2xl:px-20 mt-16 md:mt-24">
        ...
        {isConcertsLoading ? (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-8">
            {[1,2,3,4].map(i => <div key={i} className="h-96 bg-gray-200  rounded-[2rem] animate-pulse"></div>)}
          </div>
        ) : concerts.length === 0 ? (
          <div className="text-center py-20 bg-canvas  rounded-[2rem] shadow-sm border border-gray-200 ">
            <Ticket className="w-16 h-16 mx-auto text-gray-300  mb-4" />
            <p className="text-gray-500  font-medium text-lg">ยังไม่มีคอนเสิร์ตในระบบขณะนี้</p>
          </div>
        ) : (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-8">
            {concerts.map((c: any) => (
              <div key={c.id} className={`group apple-glass rounded-[2rem] overflow-hidden flex flex-col h-full transition-all duration-500 ${!c.is_active ? 'opacity-70 grayscale' : 'hover:shadow-2xl hover:-translate-y-2'}`}>
                ...
                <div className="p-6 flex flex-col flex-1 bg-canvas/50 /50">
                  ...
                  <div className="mt-auto pt-4">
                    {c.is_active ? (
                      <Link to={`/concerts/${c.access_code}`} className="flex justify-center items-center w-full bg-black  text-white  font-semibold py-3.5 rounded-full transition-all duration-300 hover:scale-105 shadow-sm">
                        ดูรายละเอียด & จองตั๋ว
                      </Link>
                    ) : (
                      <button disabled className="w-full bg-gray-100  text-gray-400  font-semibold py-3.5 rounded-full cursor-not-allowed">
                        รอเปิดจำหน่าย
                      </button>
                    )}
                  </div>
            ...
            {documents.map((d: any) => (
              <div key={d.id} className="group apple-glass rounded-[2rem] overflow-hidden flex flex-col h-full hover:shadow-2xl transition-all duration-300">
                ...
                <div className="p-6 flex flex-col flex-1 bg-canvas/50 /50">
                  ...
                  <div className="mt-auto pt-4">
                    <Link to={`/documents/${d.id}`} className="flex justify-center items-center w-full bg-gray-100 hover:bg-gray-200  dark:hover:bg-gray-700 text-ink  font-semibold py-3.5 rounded-full transition-colors duration-300">
