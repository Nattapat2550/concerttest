    ...
    <div className="max-w-4xl mx-auto p-6 mt-8 min-h-[60vh]">
      ...
      {bookings.length === 0 ? (
        <div className="bg-white dark:bg-gray-800 p-10 text-center rounded-xl shadow border dark:border-gray-700">
          <p className="text-gray-500 text-lg">คุณยังไม่มีประวัติการจองตั๋ว</p>
        </div>
      ) : (
        <div className="space-y-4">
          {bookings.map((b) => (
            <div key={b.id} className={`flex flex-col sm:flex-row justify-between items-center bg-white dark:bg-gray-800 p-6 rounded-xl shadow-md border-l-8 ${b.status === 'confirmed' ? 'border-green-500' : 'border-gray-400'}`}>
              <div className="text-center sm:text-left w-full">
                <h3 className={`text-2xl font-bold ${b.status === 'cancelled' ? 'text-gray-400 line-through' : 'dark:text-white'}`}>{b.concert_name}</h3>
                <div className="mt-2 text-lg dark:text-gray-300">ที่นั่ง: <span className="font-black text-blue-600 bg-blue-50 px-3 py-1 rounded ml-2">{b.seat_code}</span></div>
                <p className="mt-2 text-sm text-gray-500 font-bold">ยอดชำระ: ฿{b.price}</p>
              </div>
              <div className="mt-4 sm:mt-0 text-center w-full sm:w-auto flex flex-col items-center sm:items-end gap-3">
                <span className={`px-4 py-2 rounded-full text-sm font-bold shadow-sm ${b.status === 'confirmed' ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800'}`}>
