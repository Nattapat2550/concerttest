...
export default function ConcertDetailsPage() {
 ...
 if (loading) {
 return (
 <div className="flex flex-col justify-center items-center min-h-[80vh] bg-canvas">
 <img src={waitImg} alt="Loading" className="w-16 h-16 animate-spin opacity-50 dark:invert mb-6" />
 <p className="text-muted font-bold text-lg tracking-widest animate-pulse">LOADING EXPERIENCE...</p>
 </div>
 );
 }
 ...
 const rawHTML = concert.description ? decodeHTMLEntities(concert.description) : '<div class="text-center py-20 text-muted font-bold text-lg border-2 border-dashed border-outline rounded-lg">เตรียมพบกับรายละเอียดความสนุกเร็วๆ นี้</div>';
 ...
 return (
 <div className="bg-canvas min-h-screen pb-36 relative selection:bg-primary selection:text-white w-full">
 ...
 src={concert.layout_image_url} 
 alt={concert.name} 
 className="max-h-full max-w-full rounded-lg shadow-2xl drop-shadow-[0_20px_50px_rgba(0,0,0,0.5)] border border-white/10" 
 />
 </div>
 </>
 ) : (
 ...
 <div className="bg-canvas rounded-lg shadow-[0_20px_50px_rgb(0,0,0,0.1)] p-8 md:p-14 lg:p-16 border border-outline backdrop-blur-xl">
 
 <div className="text-center mb-12">
 <span className="inline-block px-4 py-1.5 rounded-full bg-primary/10 text-primary font-black text-sm uppercase tracking-widest mb-6 border border-primary/20">
 Concert Detail
 </span>
 <h1 className="text-4xl md:text-5xl lg:text-6xl font-black text-ink leading-tight mb-8">
 {concert.name}
 </h1>
 
 <div className="flex flex-col md:flex-row justify-center items-center gap-4 md:gap-6">
 <div className="flex items-center text-ink font-bold bg-canvas border border-outline px-6 py-3.5 rounded-lg shadow-sm">
 ...
 {/* HTML Content */}
 <div 
 className="prose prose-lg dark:prose-invert md:prose-xl max-w-none dark:prose-invert 
 ...
 prose-img:rounded-lg prose-img:shadow-2xl prose-img:mx-auto prose-img:my-12 prose-img:border prose-img:border-outline
 prose-iframe:w-full prose-iframe:aspect-video prose-iframe:rounded-lg prose-iframe:shadow-2xl prose-iframe:my-10"
 ...
 <div className="fixed bottom-0 left-0 w-full bg-canvas/90 backdrop-blur-xl border-t border-outline shadow-[0_-10px_40px_rgba(0,0,0,0.05)] z-50 animate-fade-in-up">
 ...
 <div className="w-16 h-16 rounded-lg bg-canvas border border-outline overflow-hidden shrink-0">
 {concert.layout_image_url && <img src={concert.layout_image_url} className="w-full h-full object-cover" alt="Thumb"/>}
 </div>
 <div>
 <h3 className="text-xl font-black text-ink line-clamp-1 max-w-lg">{concert.name}</h3>
 <p className="text-muted font-bold mt-1">เริ่มต้น <span className="text-primary text-xl">฿{concert.ticket_price?.toLocaleString() || '0'}</span></p>
 </div>
 </div>
 
 <div className="w-full sm:w-auto shrink-0 flex items-center gap-4">
 <div className="sm:hidden text-left flex-1">
 <p className="text-xs text-muted uppercase font-bold">ราคาเริ่มต้น</p>
 <p className="text-xl font-black text-primary">฿{concert.ticket_price?.toLocaleString() || '0'}</p>
 </div>
 {concert.is_active ? (
 <Link 
 ...
 <button disabled className="flex items-center justify-center bg-canvas text-muted font-black text-lg py-4 px-12 rounded-lg cursor-not-allowed border-2 border-outline border-dashed w-full sm:w-auto">
