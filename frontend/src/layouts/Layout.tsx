...
export default function Layout() {
 ...
 const isMobileActive = (path: string) => location.pathname.includes(path) ? "text-primary font-black block px-4 py-3 rounded-lg bg-primary/10" : "text-muted hover:text-primary font-medium transition block px-4 py-3 rounded-lg hover:bg-canvas";
 ...
 <div className="p-2 bg-primary/10 rounded-lg mr-3 group-hover:scale-105 transition-transform">
 ...
 {role === 'admin' && (
 <Link to="/admin" className="bg-linear-to-r from-brand to-purple-600 px-5 py-2 rounded-lg text-white font-bold shadow-lg shadow-brand/30 hover:shadow-brand/50 transform hover:-translate-y-0.5 transition-all">
 Admin Workspace
 </Link>
 )}
 </div>
 ...
 <Link to="/register" className="bg-text-main text-bg-main px-6 py-2.5 rounded-lg hover:opacity-90 transition-opacity shadow-md">สมัครสมาชิก</Link>
 </div>
 ) : (
 ...
 {dropdownOpen && (
 <div className="absolute right-0 mt-4 w-56 bg-canvas rounded-lg shadow-2xl py-2 border border-outline transition-all z-50 animate-fade-in-up">
 ...
 className="lg:hidden p-2 rounded-lg text-muted hover:text-primary bg-canvas focus:outline-none transition-colors"
 >
 ...
 {role === 'admin' && (
 <Link to="/admin" className="block px-4 py-3 mt-4 bg-linear-to-r from-brand to-purple-600 rounded-lg font-bold text-white shadow-md text-center">Admin Workspace</Link>
 )}
 {role === 'guest' && (
 <div className="pt-6 mt-4 border-t border-outline grid grid-cols-2 gap-4">
 <Link to="/login" className="flex items-center justify-center px-4 py-3 text-sm font-bold bg-canvas text-ink rounded-lg">เข้าสู่ระบบ</Link>
 <Link to="/register" className="flex items-center justify-center px-4 py-3 text-sm font-bold bg-text-main text-bg-main rounded-lg">สมัครสมาชิก</Link>
 </div>
 )}
 </div>
 </div>
