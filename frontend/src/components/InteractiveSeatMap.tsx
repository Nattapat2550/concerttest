...
export default function InteractiveSeatMap({
 ...
 // 💡 [สถานะ เลือก/Wait/Lock]
 if (selectedSeat && selectedSeat.seat_code === seatId) {
 seatData.node.style.setProperty('fill', 'var(--canvas, #ffffff)', 'important');
 seatData.node.style.setProperty('stroke', '#ef4444', 'important');
 seatData.node.style.setProperty('stroke-width', '2px', 'important');
 seatData.node.style.cursor = 'pointer';
 } else if (mode === 'booking' && waitSet.has(seatId)) {
 ...
 }, [configuredSeats, bookedSeats, combinedWaitSeats, mode, selectedSeat]); 
 ...
 <button onClick={() => handleZoom(-0.5)} className="bg-lifted px-3 py-1 rounded font-bold hover:bg-gray-300 transition">-</button>
 <button onClick={handleReset} className="bg-lifted px-3 py-1 rounded text-sm font-bold hover:bg-gray-300 transition">RESET</button>
 <button onClick={() => handleZoom(0.5)} className="bg-lifted px-3 py-1 rounded font-bold hover:bg-gray-300 transition">+</button>
 </div>
 </div>

 <div className={`absolute top-4 left-1/2 -translate-x-1/2 bg-slate-900/80 text-white px-5 py-2.5 rounded-full shadow-lg pointer-events-none z-20 font-bold text-sm transition-all duration-300 transform ${showZoomHint ? 'opacity-100 translate-y-0' : 'opacity-0 -translate-y-2'}`}>
 ...
 <p className="text-muted font-bold text-2xl text-center">แอดมินยังไม่ได้เปิดขายที่นั่ง</p>
 </div>
 ) : svgContent ? (
 <div ref={transformWrapperRef} className="w-full h-full flex items-center justify-center" />
 ) : (
 <div className="flex justify-center items-center w-full h-full absolute">
 <p className="text-muted font-bold text-xl pointer-events-none">ไม่มีแผนผัง Interactive</p>
 </div>
 )}
 </div>
 </div>
